...
void set_options(pcap_t *dev,  const int snaplen, const int promisc, const int rfmon, const int to_ms, const int buffer_size, const int tstamp_type) {
	int ret;
	...
	if (ret == PCAP_ERROR_ACTIVATED ) {
		...
		fprintf(stderr, "Error: time stamp type not supported");
		exit(EXIT_FAILURE);
	}
}
...
int analyze_packets(pcap_t *p, u_char *user) {
	return pcap_loop(p, -1, packet_handler, user);	/* process packets infinitely until call pcap_breakloop */
}
